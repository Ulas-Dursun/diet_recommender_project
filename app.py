...
inputs = [
    ...
    gr.Dropdown(["None","Diabetes","Hypertension","Cardiac"], label="Disease Type"),
    gr.Dropdown(["Low","Moderate","High"], label="Severity"),
    gr.Number(label="Cholesterol (mg/dL)"),
    gr.Number(label="Blood Pressure (mmHg)"),
    gr.Number(label="Glucose (mg/dL)"),
    gr.Dropdown(["Low","Medium","High"], label="Physical Activity Level"),
    gr.Number(label="Weekly Exercise Hours"),
    gr.Dropdown(["None","Vegetarian","Vegan","Gluten_Free"], label="Dietary Restrictions"),
    gr.CheckboxGroup(
        ["Dairy","Eggs","Fish","Gluten","Peanuts","Shellfish","Soy","Tree_Nuts"],
        label="Allergies (select all that apply)"
    ),
    gr.Dropdown(["Turkish","Asian","Mediterranean","Other"], label="Preferred Cuisine"),
    gr.Number(label="Daily Caloric Intake"),
    gr.Slider(1, 10, label="Adherence to Diet Plan")
]

outputs = [
    gr.Textbox(label="Recommended Diet Plan"),
    gr.Markdown(label="Suggested Recipes")
]
